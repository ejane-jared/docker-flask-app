...
docker-flask-app
  ...
  └── README.md
```
The following steps in this document will provide guidance on writing the contents of each file necessary to build and run the application with docker.
...
In your command line, while under the project folder root, enter the following command:
```
python -m flask run
```
...
The requirements.txt contains a list of packages or libraries needed for a project. 
...
In the above case where the Flask version is 3.1.0, the requirements.txt file will contain the following:
```
Flask==[3.1.0]
```

Alternatively, you can run `pip freeze > requirements.txt`
This command gathers all requirements for the flask app and place into requirements.txt
